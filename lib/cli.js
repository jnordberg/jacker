...
function main() {
  ...
  function write(contents) {
    ...
    out.write(contents, function(error) {
      if (error) throw error;
        ...
        log('ok\n');
        if (out.bytesWritten != undefined) // https://github.com/joyent/node/issues/2317
  ...
  if (argv['show-tree']) {
    var tree = {}, base = process.cwd()
    async.forEach(scripts, function(script, callback) {
      packager.buildDependencyTree(script, base, tree, callback);
    }, function(error) {
      if (error) throw error;
      log('dependency tree:');
